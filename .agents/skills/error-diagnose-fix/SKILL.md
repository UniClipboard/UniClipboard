...
```bash
# Example for cargo
cargo check --workspace --locked 2>&1 | tee /tmp/codex-edf-build-output.txt
echo "EXIT_CODE=$?"
```
...
git rev-parse HEAD > /tmp/codex-edf-revert-point.txt
```
...
```bash
rm -f /tmp/codex-edf-state.json /tmp/codex-edf-build-output.txt /tmp/codex-edf-revert-point.txt
```
