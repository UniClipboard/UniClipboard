...
- `$pr-greenlight` - full loop: preflight + create PR + monitor
- `$pr-greenlight preflight` - only run local pre-flight checks, no push
- User says "帮我把这个 PR 搞绿", "run preflight", "pre-flight checks", "搞定 PR"

## State file

`/tmp/codex-pr-greenlight-state.json`:
...
Lock file: `/tmp/codex-pr-greenlight.lock`

## Phase 1 — Perceive: Analyze what changed

```bash
touch /tmp/codex-pr-greenlight.lock
```
...
If the user only asked for `$pr-greenlight preflight`, **stop here**. Clean up the lock file and report results.
...
If no checks have registered, keep the current turn active, wait with the available command/session wait mechanism, and check again. Do not end the task merely because CI has not appeared yet.
...
- **Some pending** -> continue monitoring in the same turn
...
After pushing a fix, prefer `gh pr checks --watch --interval 30` and poll its command session in bounded intervals. Re-enter at Phase 4b when the checks finish. Keep the current Codex turn alive and continue reporting useful status while waiting.

## Cleanup

**On success:**
```bash
rm -f /tmp/codex-pr-greenlight.lock /tmp/codex-pr-greenlight-state.json
```
```text
...
- If the user invokes `$pr-greenlight preflight`, only Phase 1-2 execute (no push, no PR)
