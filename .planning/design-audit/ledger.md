...
|----|----------|------|-------|--------|-----------|-----------|------|
| uc-application:apply_inbound:index_for_search:L7 | P2 | L7 | snapshot 二次深拷贝 (OS write clone 后 index_for_search 再 clone)，图片 MB 级 | fixed | 2026-W26 | 2026-W26 | S0：snapshot_for_write 改 Arc 共享给 index + OS write(try_unwrap)，对齐 watcher；**working tree 未 commit**，29 ApplyInbound 测试绿 |
...
| uc-application:projection:build_mime_loop:L4 | P2 | L4 | build_from_capture/build_from_persisted ~80 行 MIME 解析逐字重复 | fixed | 2026-W26 | 2026-W26 | S0：抽 SearchableContent::ingest + into_pipeline_input，两方法各 ~18 行；**working tree 未 commit**，663+9 测试绿 |
