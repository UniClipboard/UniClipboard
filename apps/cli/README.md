...
| `uniclip space status`              | 查看当前空间、本机成员身份和相关状态。                                                                                                                                                                                                                   |
...
| `uniclip space reset --yes`         | 保留本机历史、文件、设置、身份和解锁信息，永久废弃全部旧设备关系，并重建为仅含本机的空间。所有设备都需要重新配对。                                                                                                                                       |
...
旧的顶层 `status`、`init`、`invite`、`join` 和 `reset-space` 入口暂时保留，
每次执行都会提示对应的 `space` 新命令。新脚本和文档不得继续使用旧入口。
