...
fn desktop_profile_partition_does_not_change_the_space_key_scope() {
    ...
    assert!(source.contains("with_portable_storage(uc_app_paths::is_portable())"));
    assert!(
...
fn daemon_member_handlers_delegate_roster_ownership_to_engine() {
    ...
    assert!(
        !member.contains(".member_roster")
            ...
            && member.contains("Operation::QueryDeviceGroupChoices")
            && member.contains("Operation::ChooseDeviceGroup"),
        "daemon member preference and device group handlers must delegate to uc-engine"
    );
    assert!(
