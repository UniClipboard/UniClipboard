...
impl Engine {
    ...
    pub async fn execute(&self, operation: Operation) -> Result<OperationResult, EngineError> {
        ...
        let result = tokio::select! {
            ...
            result = self.runtime.execute(operation, cancellation.clone()) => result,
        };
    ...
    pub async fn resume(&self) -> Result<(), EngineError> {
        let _lifecycle = self.lifecycle_gate.lock().await;
        if self.state.lock().await.lifecycle != EngineState::Suspended {
            return Err(invalid_state_error());
        }
    ...
    pub async fn shutdown(&self, deadline: Duration) -> Result<(), EngineError> {
        ...
        self.state.lock().await.lifecycle = EngineState::ShuttingDown;
        self.events.send(EngineEvent::StateChanged {
        ...
        self.state.lock().await.lifecycle = EngineState::Stopped;
        self.events.send(EngineEvent::StateChanged {
    ...
    async fn quiesce_locked(&self, deadline: Duration) -> Result<(), EngineError> {
        {
            let mut state = self.state.lock().await;
            if state.lifecycle != EngineState::Running {
                return Err(invalid_state_error());
            }
            state.lifecycle = EngineState::Quiescing;
        }
        self.events.send(EngineEvent::StateChanged {
        ...
        self.state.lock().await.lifecycle = EngineState::Quiesced;
        self.events.send(EngineEvent::StateChanged {
