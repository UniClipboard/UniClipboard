...
//! [`crate::internal::deps::WiredDependencies`] as input and does **not** re-run
...
use crate::internal::deps::{SharedRuntimeDeps, SyncEngineDeps};
use crate::internal::sync_engine::{build_sync_engine_assembly, SyncEngineAssembly};
...
/// reconcile。接受已 wire 好的进程级 [`crate::internal::deps::WiredDependencies`] 作输入,
...
pub async fn build_daemon_lifecycle(
    ...
    // 启动 —— reconcile 是治理性的。
    if let Err(err) = crate::internal::reconcile::reconcile_peer_addresses(
    ...
    if let Err(err) = crate::internal::reconcile::reconcile_trusted_peers(
        Arc::clone(&deps.device.member_repo),
    ...
    let mut iroh_config = crate::internal::network_policy::relay_policy_to_iroh_config(
    ...
    crate::internal::network_policy::apply_iroh_direct_reachability_from_env(&mut iroh_config);
    crate::internal::network_policy::apply_congestion_controller_from_env(&mut iroh_config);
