...
//!   因为 telemetry 阻塞或出错。
//! - **不传宿主事件上下文**：实现者在构造时持有 context。这样：
//!   - 调用方只关心"发生了什么事"，不关心"我是谁"；
//!   - context 字段的命名冲突由 sink 负责仲裁。
//! - **gate 责任在调用方**：进入 `capture` / `identify` 之前调用方应查询
//!   宿主提供的启用状态。`NoopAnalyticsSink`
...
///
/// 实现者必须：
/// - 在内部持有一份宿主事件上下文，与每条事件 properties
///   合并后再上传；
