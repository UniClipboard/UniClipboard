...
Without `--no-wait`, `uniclip space join` waits for a final result. Ctrl-C stops
only that local wait; use `space join cancel` when the request itself should be
cancelled.
...
| Command                      | Effect                                                                                         |
| ---------------------------- | ---------------------------------------------------------------------------------------------- |
| `uniclip space reset --yes` | Rebuild this profile as a one-device space while retaining its local history and settings.   |

The reset permanently removes every old device relationship from this profile.
It does not erase the other devices; pair every device again before sync resumes.
