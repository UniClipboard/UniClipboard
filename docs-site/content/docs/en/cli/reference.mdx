...
| `uniclip space join`        | Join or re-pair with a sponsoring device; add `--switch` to move to a different space.                        |
...
Without `--no-wait`, `uniclip space join` waits for a final result. Ctrl-C stops only
...
`uniclip member list` to see the available names. `--detailed` (and `--json`)
