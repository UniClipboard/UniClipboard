...
double-click `UniClipboard.exe`. It requires no installation and writes no
registry entries. Upgrade recovery copies are the system-directory exception
described below.

- **Working data lives next to the app.** The encrypted database, keys, search
  index, and logs are stored in a `data` folder created next to
  `UniClipboard.exe`. Move the whole folder (exe + `portable.dat` + `data`) to
  carry your setup; delete `data` to reset. Recovery copies created before a
  profile upgrade remain under `%LOCALAPPDATA%` so replacing or deleting the
  portable folder cannot remove the last usable copy.
...
<Callout type="warn">
  **Known limitation:** the WebView2 runtime's browser cache is still written under `%LOCALAPPDATA%`
  ...
  Besides the browser cache, pre-upgrade recovery copies are intentionally kept under
  `%LOCALAPPDATA%`. Your active data (database, keys, search index, and logs) remains in the
...
Arch Linux and Omarchy users can install the community-maintained binary package directly, avoiding a local build:

```bash
yay -S uniclipboard-bin
```

Alternatively, install the upstream-maintained source package, which tracks the `main` branch:

```bash
yay -S uniclipboard-git
```

`uniclipboard-bin` is maintained by the community and repackages UniClipboard's prebuilt release; `uniclipboard-git` is maintained upstream but builds the application locally. Both install the GUI, daemon, desktop entry, and the GTK Layer Shell runtime required by the Quick Panel on Wayland. Omarchy delegates global shortcuts to Hyprland, so complete the one-time setup under [Quick Panel — Omarchy](../core-features/quick-panel#omarchy) after installation. The installer does not currently modify personal Hyprland configuration, so Quick Panel activation is not yet zero-configuration on Omarchy.
...
For servers, SSH sessions, or anything without a desktop, follow the dedicated
[CLI installation guide](../cli/installation). The CLI package includes both
`uniclip` and its separate `uniclipd` background service; keep them together.
No GUI is required.
...
The Windows **portable** build keeps active data in a `data\` folder next to
`UniClipboard.exe`. Pre-upgrade recovery copies remain under
`%LOCALAPPDATA%\app.uniclipboard.desktop[-<profile>]-upgrade-backups\` so they
survive replacement or deletion of the portable folder. See the "Portable
build" section under the Windows tab above.
