...
<Callout type="info">
  ...
  installs and, when you click **Check for updates**, pops a dialog with the exact
  `apt` / `dnf` command (and a copy button) instead of attempting an in-app install. AppImage on
  Linux and `.app` on macOS continue to update from inside the app. For Windows, see the
  **Windows** tab.
</Callout>
