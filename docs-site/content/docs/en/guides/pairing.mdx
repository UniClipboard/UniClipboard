...
The first device runs `uniclip space init` (CLI) or **This is my first device** (GUI).
That step:
...
`uniclip space invite` (sponsor) prints a short-lived, one-time invitation code. The
joiner runs `uniclip space join --code <code>` and supplies the same passphrase. Under the
hood:
...
3. On success, the sponsor wraps the MasterKey under a fresh KEK
   ...
   it the same way the sponsor did during `uniclip space init`.
4. Both devices add each other to their **paired devices** list.

While the other device is still completing the exchange, the Devices page shows
**Waiting for peer confirmation**. Once both sides confirm, the normal connection
status returns. If confirmation does not arrive before the pairing window closes,
the device remains listed as **Unconfirmed** so you can remove it manually.
...
<Callout type="info">
  Lost a device? Remove it from any other device in the space (GUI: **Devices** page;
  CLI: `uniclip member remove <PEER-ID>`).
