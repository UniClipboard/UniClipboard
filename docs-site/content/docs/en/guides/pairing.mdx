...
The first device runs `space init` (CLI) or **This is my first device** (GUI).
That step:
...
`space invite` (sponsor) prints a short-lived, one-time invitation code. The
...
3. On success, the sponsor wraps the MasterKey under a fresh KEK
   ...
   it the same way the sponsor did during `space init`.
