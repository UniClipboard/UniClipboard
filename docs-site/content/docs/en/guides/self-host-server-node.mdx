...
<Callout type="info">
  `/healthz` needs no authentication, by design. It returns an empty `204`
  with no version, device, username, or clipboard information — it reveals
  only that a listener answers at this address, which a TCP connect already
  reveals. The real abuse surface would be the opposite: making every probe
  run a full Argon2id verification (what `/SyncClipboard.json` costs per
  request — roughly 64MB of memory and tens of milliseconds of CPU).
</Callout>

<Callout type="warn">
  Don't rewrite or proxy the daemon control API's `GET /health` to
  `/healthz`. They are different endpoints on different listeners: control
  `/health` is on the loopback API, `/healthz` is on the mobile LAN
  listener. Passing one off as the other makes the probe report success
  while the mobile path is actually unreachable.
</Callout>
