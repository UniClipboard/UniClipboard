...
<Callout type="info">
  ...
  SyncClipboard-compatible (`GET /SyncClipboard.json` + HTTP Basic Auth). See
  [Mobile LAN API](./mobile-api) for the endpoints your client will call after
  parsing the URI.
</Callout>
...
Any change that *removes* or *re-types* a required v1 field requires bumping
payload `v`.
