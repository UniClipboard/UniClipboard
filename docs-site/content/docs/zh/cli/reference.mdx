...
| `uniclip space join`        | 加入或重新配对 sponsor；加 `--switch` 可切换到另一个空间。                         |
...
不带 `--no-wait` 时，`uniclip space join` 会等待最终结果。Ctrl-C 只停止本地等待；
...
`--source-device` 按「内容来自哪台设备」过滤，接受设备名（大小写无关）或设备 id，可重复多次；运行 `uniclip member list` 可查看可用的设备名。`--detailed`（以及 `--json`）的输出里会带上每条结果的 `source` 设备 id。
...
- **写命令在同 profile daemon 跑着时会被拒绝。** 跑 `setup`、`add`、
  `network set` 等之前先 `uniclip stop` / 退掉 GUI。读命令（`space status`）在
  daemon 跑着时也可以。
