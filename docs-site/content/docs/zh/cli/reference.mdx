...
位置参数对应现有普通文件时会自动按文件发送。使用 `--text` 可强制把文件名作为文字
发送，使用 `-f` / `--file` 可强制进入文件模式。暂不支持目录；明显但不存在的路径会
直接报错，不会静默当作文字。文件模式支持重复使用 `--peer`；所有相关目标均已完成、
失败或离线后命令会自动退出，Ctrl-C 可取消等待。`recv` 成功时，stdout 只包含收到文件的
绝对路径；进度仍以交互方式显示，`--json` 则返回完整结果对象。
