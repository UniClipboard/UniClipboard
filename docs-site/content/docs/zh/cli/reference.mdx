...
| 命令                     | 作用                                             |
| ------------------------ | ------------------------------------------------ |
| `uniclip join --no-wait` | 记录等待中的加入请求后立即返回，不等待最终结果。 |
| `uniclip join status`    | 查看当前加入请求及其 Engine 状态。               |
| `uniclip join cancel`    | 请求取消当前仍在等待的加入。                     |

不带 `--no-wait` 时，`uniclip join` 会等待最终结果。Ctrl-C 只停止本地等待；
确实要取消请求时应使用 `join cancel`。

### 设备信任变化

| 命令                          | 作用                                                   |
| ----------------------------- | ------------------------------------------------------ |
| `uniclip member trust status` | 查看当前设备组变化、变化编号以及每种选择的影响。       |
| `uniclip member trust apply`  | 确认后应用等待处理的设备组变化。                       |
| `uniclip member trust keep`   | 保留当前设备组，不应用等待处理的变化。                 |

脚本和 JSON 调用必须传入 `--change <CHANGE-ID>`，确保选择对应刚刚查看的变化。
如果应用变化会移除本机，还必须传入 `--confirm-local-removal`。
...
`<DEVICE>` 可以是设备编号；在交互终端中也可以使用没有歧义的设备名称。
