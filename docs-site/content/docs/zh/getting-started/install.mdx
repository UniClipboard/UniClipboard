...
<Callout type="warn">
  UniClipboard 处于积极开发的 alpha 阶段。功能会持续演进，跨版本升级时同步状态可能会失效。请把它当成"好用但还不到当作基础设施依赖"的状态。
</Callout>
...
<Callout type="info">
  首次运行新版本时 SmartScreen 可能弹出警告。如果发布者信息与签名身份一致，点击
  **更多信息 → 仍要运行**。如果不放心，可以在 **属性 → 数字签名** 中校验签名。
</Callout>
