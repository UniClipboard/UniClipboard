...
如果问题出现在从 0.19 升级之后，请先看[从 0.19 升级到 1.0](../migration/upgrade-0-19-to-v1)。它说明资料备份、搜索重建、设备列表清空和重新配对的正常过程。

<Callout type="info">
  在排查任何问题之前，先在两端各跑一遍 `uniclip space status` 与 `uniclip member list` 拿到客观状态。
  人脑记忆里的"应该在线"经常和实际不符。
</Callout>
...
CLI 的所有同步功能都依赖守护进程。`uniclip start` 拉起、`uniclip stop` 停掉、`uniclip space status` 看健康度。

```bash
uniclip space status            # 摘要
uniclip space status --json     # 字段稳定，方便比对
```
...
先看前台进程打印的具体错误。如果它找不到或无法启动 `uniclipd`，请重新安装 CLI，
或把同版本的 `uniclip` 与 `uniclipd` 放回同一个目录。桌面应用和 CLI 通常可以
共用一个 profile 和同一个后台服务。
...
```bash
uniclip space invite        # 在 sponsor 上重出一张
```
...
2. 在 sponsor 端 `uniclip space status --json | grep handshake` 查看是否在等 joiner，如果根本没收到说明发现层就没打通，跳到[对端不上线](#对端不上线)。
...
<Callout type="warn">
  丢失的口令无法查看或找回。如果本机已经解锁，并且是空间中的唯一设备，可以直接点击邀请窗口里的
  **重置口令**，或前往 **设置 → 安全与隐私** 自定义新口令，本机历史会保留。如果空间里有多台设备，
  需要先移除其他设备；如果没有任何设备能解锁空间，则只能重置并重新开始。
</Callout>
...
- 防火墙白名单需要放开 iroh 默认的 rendezvous + 中继域名。具体域名见 `uniclip space status --json` 的 `network` 字段。
...
`uniclip space status` 里 `network.relay` 非空就说明走的是中继 —— 能跑，但延迟和带宽都不如直连。如果你确定双方在同一个网络下却仍然走中继：
...
- **"index not ready"** —— 守护进程刚启动 / 大版本升级后正在重建。`uniclip search status` 会给出进度。
...
- `uniclip space status --json`（两端各一份）。
- 复现步骤与时间戳，对应时间窗口的日志切片即可，不必整份上传。
- **不要** 把数据库文件、邀请码、空间口令贴到 issue 里。

## 紧急重置（最后手段）

如果只是系统保存的密钥丢失，但仍记得原加密口令，先按[恢复本机资料](../migration/upgrade-0-19-to-v1#提示恢复本机资料)尝试恢复。恢复页没有一键清空入口；不要把重置当作恢复的必要步骤。
...
下面的入口只在普通内容解锁页提供，不在“恢复本机资料”、首次启动或升级页面提供。关闭自动解锁只会隐藏内容和管理设置，不会停止正常后台同步。忘记口令时，请先联系作者并备份，再考虑重置：
...
清理后 `uniclip space init` 或 `uniclip space join` 即可重来。系统密钥环里的 KEK 条目也会在下次 `space init` 时被覆盖。
