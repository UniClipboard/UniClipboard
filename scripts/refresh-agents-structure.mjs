...
const KNOWN_DESCRIPTIONS = {
  ...
  "uc-observability":
    "Dual-output tracing, profile filtering, Sentry/analytics scope",
  "uc-bootstrap":
  ...
  "uc-webserver":
    "Daemon's 127.0.0.1 HTTP + WebSocket API (OpenAPI / ApiEnvelope)",
  "uc-daemon-contract": "Transport DTOs/contracts shared by client + server",
  "uc-daemon-process":
    "Thin process primitives: PID file, socket path, spawn, health-wait",
  "uc-daemon":
    "GUI-agnostic daemon runtime; hosts the `uniclipd` binary",
  "uc-daemon-local":
    "Local process coordination: auth token, socket discovery, health polling",
  "uc-daemon-client": "Daemon HTTP + WS client (used by GUI + CLI)",
  "uc-desktop":
    "Desktop host: runtime, daemon probe, background tasks (GUI-framework-agnostic)",
  "uc-tauri":
    "Tauri adapter: commands (via tauri-specta), tray, quick panel, run loop",
...
function main() {
  ...
  const newContent = updateRefreshDate(
    replaceSection(content, structure),
    members.length,
  );
  ...
  console.log(
    `[refresh-agents-structure] Updated STRUCTURE (${members.length} crates)`,
  );
}
