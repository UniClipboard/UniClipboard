    ...
    pub fn build_postings(
        ...
        for (segments, field_bit) in fields {
            ...
            for segment in *segments {
                ...
                let raw_token_counts = count_raw_tokens(&self.tokenizer.tokenize_segment(segment), segment);
        ...
        postings.sort_by(|a, b| a.term_tag.cmp(&b.term_tag).then(a.field_mask.cmp(&b.field_mask)));
...
mod tests {
    ...
    fn pipeline_aggregates_repeated_hits_into_one_posting_with_increased_term_freq() {
        ...
        assert!(max_freq >= 3, "expected term_freq >= 3 for repeated token, got max {max_freq}");
    }
    ...
    fn pipeline_document_deduplicates_file_extensions() {
        ...
        let doc = pipeline.build_document(&input);
        assert_eq!(doc.file_extensions, vec!["md".to_string(), "txt".to_string()]);
    }
