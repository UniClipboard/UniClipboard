...
mod tests {
    ...
    fn plain_text_wins_over_html() {
        ...
        assert!(result.html.is_empty(), "html field should be empty when plain_text is present");
    }
    ...
    fn html_visible_text_used_when_plain_text_absent() {
        ...
        let result = ext.extract(&input);
        assert!(result.body.is_empty(), "body should be empty when plain_text is absent");
    ...
    fn url_extraction_includes_host_path_query_key_names_but_not_values() {
        ...
        let input = SearchPipelineInput {
            uri_list: vec![
                "https://github.com/rust-lang/rust?q=search&tab=code".to_string()
            ],
            ..base_input()
        };
        let result = ext.extract(&input);
        // Host
        assert!(result.url.contains(&"github.com".to_string()), "should contain host");
        // Path segments
        assert!(result.url.contains(&"rust-lang".to_string()), "should contain path segment");
        assert!(result.url.contains(&"rust".to_string()), "should contain path segment");
        // Query param names
        assert!(result.url.contains(&"q".to_string()), "should contain query key name");
        assert!(result.url.contains(&"tab".to_string()), "should contain query key name");
        // Query values should NOT be present
        assert!(!result.url.contains(&"search".to_string()), "should NOT contain query value");
        assert!(!result.url.contains(&"code".to_string()), "should NOT contain query value");
    }
    ...
    fn file_path_extraction_includes_directory_segments_stem_and_extension() {
        ...
        assert!(result.file_path.contains(&"report.pdf".to_string()), "whole filename");
