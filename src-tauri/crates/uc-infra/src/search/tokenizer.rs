...
mod tests {
    ...
    fn tokenizer_emits_lowercase_nfkc_tokens() {
        ...
        let all = tokens.join(" ");
        assert!(all.contains("hello") || all.contains("world") || all.contains("helloworld"),
    ...
    fn tokenizer_preserves_whole_identifier_and_split_tokens() {
        ...
        assert!(tokens.iter().any(|t| t == "baz"), "expected 'baz' in {tokens:?}");
        assert!(tokens.iter().any(|t| t == "qux"), "expected 'qux' in {tokens:?}");
        assert!(tokens.iter().any(|t| t == "txt"), "expected 'txt' in {tokens:?}");
    ...
    fn tokenizer_drops_single_char_latin_noise() {
        ...
        assert!(!tokens.contains(&"a".to_string()), "'a' should be dropped: {tokens:?}");
        assert!(!tokens.contains(&"b".to_string()), "'b' should be dropped: {tokens:?}");
        assert!(!tokens.contains(&"c".to_string()), "'c' should be dropped: {tokens:?}");
    ...
    fn tokenizer_keeps_cjk_bigrams() {
        ...
        assert!(tokens.contains(&"中文".to_string()), "expected '中文' bigram in {tokens:?}");
        assert!(tokens.contains(&"文测".to_string()), "expected '文测' bigram in {tokens:?}");
        assert!(tokens.contains(&"测试".to_string()), "expected '测试' bigram in {tokens:?}");
    }
    ...
    fn tokenizer_handles_camelcase_splitting() {
        ...
        assert!(tokens.iter().any(|t| t == "get"), "expected 'get' in {tokens:?}");
        assert!(tokens.iter().any(|t| t == "user"), "expected 'user' in {tokens:?}");
        assert!(tokens.iter().any(|t| t == "name"), "expected 'name' in {tokens:?}");
    }
