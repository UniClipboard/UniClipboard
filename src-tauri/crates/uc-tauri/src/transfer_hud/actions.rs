...
//! 域类型。平台代码只调 [`TransferHudActions::cancel`] 这种"动作动词",
//! 装配代码负责把动作翻译成"乐观状态更新 + 真正向 facade 发请求"。
//!
//! 这层抽象的边界:
//! - **平台模块拿不到的**:emitter / facade / Tauri State
//! - **平台模块拿得到的**:`Arc<dyn TransferHudActions>` + 平台自有的
...
//!    模块不动。
//! 3. 测试时可以用 mock `TransferHudActions` 验证 UI 行为,不需要起
//!    真实的 facade。
...
use super::emitter::TransferHudEmitter;

/// HUD 用户动作。所有方法都是 fire-and-forget:动作内部应自行处理
/// 乐观 UI 状态、异步执行、错误日志。调用方不等待结果。
pub trait TransferHudActions: Send + Sync {
...
/// 但当前进程级单例不需要。
pub struct DefaultTransferHudActions {
    emitter: Arc<TransferHudEmitter>,
    app_handle: AppHandle,
}

impl DefaultTransferHudActions {
    pub fn new(emitter: Arc<TransferHudEmitter>, app_handle: AppHandle) -> Self {
        Self {
            emitter,
            app_handle,
        }
...
impl TransferHudActions for DefaultTransferHudActions {
    fn cancel(&self, transfer_id: &str) {
