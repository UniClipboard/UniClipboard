...
export function subscribeDesktopTheme(
  ...
  const deliver = (snapshot: DesktopThemeSnapshot) =>
    onTheme(
      snapshot.followOmarchyTheme && snapshot.omarchyAvailable ? snapshot.theme : null,
      snapshot.windowCornerRadius,
      snapshot
    )
  if (latestSnapshot) deliver(latestSnapshot)
